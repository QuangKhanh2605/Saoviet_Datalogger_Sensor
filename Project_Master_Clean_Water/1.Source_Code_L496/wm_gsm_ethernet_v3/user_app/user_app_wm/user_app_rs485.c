...
static uint8_t fevent_rs485_handle_data_recv(uint8_t event);
static uint8_t fevent_rs485_detect_connect(uint8_t event);
...
sEvent_struct               sEventAppRs485[]=
{
  ...
  {_EVENT_RS485_HANDLE_DATA_RECV,   0, 5, 10,               fevent_rs485_handle_data_recv},
  {_EVENT_RS485_DETECT_CONNECT,     1, 5, 30000,            fevent_rs485_detect_connect},
...
/*------------------Xu ly chuoi nhan duoc----------------*/
    ...
    if(sUart485.Length_u16 > 2)
    {
        ...
        if(Crc_Check == Crc_Recv)
        {
            count_success++;
            Pos = 3;
            if(sUart485.Length_u16 % 2 == 1)
            {
                length_2 = sUart485.Length_u16;
                
                Stamp_Hex = Read_Register_Rs485(sUart485.Data_a8, &Pos, 4);
                sDataSensorMeasure.spH.Scale_u8 = 0xFE;
                sDataSensorMeasure.spH.Value_i32 =  Handle_HexFloat_To_Int32_Round(Stamp_Hex, sDataSensorMeasure.spH.Scale_u8);
                
                Stamp_Hex = Read_Register_Rs485(sUart485.Data_a8, &Pos, 4);
                sDataSensorMeasure.sTemp.Scale_u8 = 0xFE;
                sDataSensorMeasure.sTemp.Value_i32 =  Handle_HexFloat_To_Int32_Round(Stamp_Hex, sDataSensorMeasure.sTemp.Scale_u8);
            }
            else
            {
                length_1 = sUart485.Length_u16;
            }
    ...
    RS485_Init_Data();
    fevent_enable(sEventAppRs485, _EVENT_RS485_DETECT_CONNECT);
    return 1;
}
